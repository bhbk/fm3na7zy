...
namespace Bhbk.Lib.Aurora.Domain.Helpers
{
    ...
    public class LogHelper : LogWriterBase
    {
        ...
        public LogHelper(ILogger logger, User user, LogLevel level)
        {
            Level = level;
            _user = user;
            _logger = logger;
        ...
        private void Write(LogLevel level, Type objectType, User user, int objectId, string area, string message, ArraySegment<byte>? data)
        {
